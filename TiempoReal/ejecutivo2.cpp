...
//  -------------
//
//   Tm = mcm(500,500,1000,2000) = 2000ms
//
//   Planificación (con Ts == 500 ms)
//  *---------*----------*---------*--------*
//  | A B C   | A B D    | C D     | A B D  |
//  *---------*----------*---------*--------*
//
//
// Historial:
...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         cout << "Tiempo de retraso: " << milliseconds_f(retraso).count() << " milisegundos" << endl;

         if (retraso > milliseconds(margen)){
           cout << "Error: Se ha excedido el máximo de " << margen << " milisegundos de retraso" << endl;
           abort();
         }
