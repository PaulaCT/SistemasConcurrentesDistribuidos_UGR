...
const int num_filosofos = 5, num_procesos = 2*num_filosofos+1, id_camarero = 0,
  num_sillas = 4;
int filosofos_mesa = 0, malaje_sentado = 1;
...
void funcion_filosofos(int id, bool hambriento){
  ...
  while ( true ){
    ...
    MPI_Ssend(&peticion, 1, MPI_INT, id_camarero, 2, MPI_COMM_WORLD);
    MPI_Recv(&valor, 1, MPI_INT, id_camarero, 2, MPI_COMM_WORLD, &estado);

    if (valor == 0 && hambriento && id != 6)
      cout << "El filósofo 6 está sentado pero el filósofo " << id << " está hambriento..." << endl;
    if ((valor == 0 && hambriento) || id == 6 || valor == 1){
      ...
      MPI_Ssend(&peticion, 1, MPI_INT, id_camarero, 3, MPI_COMM_WORLD);
    } else {
      cout << "-------------- Filosofo " << id << " se salta la comida.   --------------" << endl;
    }
...
void funcion_camarero(){
  int valor, tag_aceptable, id_aceptable;
  MPI_Status estado;
  ...
  while(true){
    ...
    switch(estado.MPI_TAG){
       ...
       if (estado.MPI_SOURCE == 6) malaje_sentado = 0;
          MPI_Ssend (&malaje_sentado, 1, MPI_INT, estado.MPI_SOURCE, 2, MPI_COMM_WORLD);
          break;

       case 3: filosofos_mesa--;
       if (estado.MPI_SOURCE == 6)  malaje_sentado = 1;
...
int main(int argc, char** argv){
   ...
   if ( num_procesos == num_procesos_actual )
   {
      ...
      if (id_propio == 0) funcion_camarero();
         ...
         funcion_filosofos( id_propio, true); //   es un filósofo
      else                               // si es impar
