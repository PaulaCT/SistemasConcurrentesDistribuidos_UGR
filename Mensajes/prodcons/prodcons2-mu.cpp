...
void funcion_buffer()
{
   int        buffer[tam_vector],      // buffer con celdas ocupadas y vacías
              ...
              tag_aceptable ;    // identificador de tag aceptable
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 ) tag_aceptable = 0;
      else if ( num_celdas_ocupadas == tam_vector ) tag_aceptable = 1;
      ...
      int emisor = estado.MPI_SOURCE;

      if (emisor < id_buffer){
        buffer[primera_libre] = valor ;
        primera_libre = (primera_libre+1) % tam_vector ;
        num_celdas_ocupadas++ ;
        cout << "                                    >>>> Buffer ha recibido valor "
          << valor <<  " del productor " << emisor << " <<<<" << endl
          << "                                    " << num_celdas_ocupadas
          << " en stock" << endl;
        ...
        num_celdas_ocupadas-- ;
        cout << "                                    >>>> Buffer va a enviar valor "
          << valor << " al consumidor " << emisor << " <<<<" << endl
          << "                                    " << num_celdas_ocupadas
          << " en stock" << endl;
        MPI_Ssend( &valor, 1, MPI_INT, emisor, 1, MPI_COMM_WORLD);
      }
   }
}
